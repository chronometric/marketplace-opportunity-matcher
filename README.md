...
This workflow is built in **Airtable**, **Make**, and **Google Sheets** — screenshots are **your** exports, not generated by this repo. GitHub only shows an image if that **exact file path exists on the branch you are viewing** (usually `main`). If you see broken icons or 404 on raw URLs, the PNG is missing, misnamed, or not pushed yet — that is expected until you add real files.

**Add images (once):**

1. Capture UI from your base and scenarios (redact keys and PII — see [`assets/screenshots/README.md`](assets/screenshots/README.md)).
2. Save PNGs into [`assets/screenshots/`](assets/screenshots/) using these **exact** names:

| Filename | What to show |
|----------|----------------|
| `airtable-raw-leads.png` | Raw Leads grid |
| `airtable-classified.png` | Classified — Supply / Demand / Pending |
| `airtable-matches.png` | Matches sorted by score |
| `make-daily-pull.png` | Make scenario — Daily Pull |
| `sheets-daily-top-10.png` | Google Sheet tab **Daily_Top_10** |

3. Commit and push: `git add assets/screenshots/*.png && git commit -m "docs: add README screenshots" && git push`

4. Paste the image grid from [`assets/screenshots/EMBED.md`](assets/screenshots/EMBED.md) into this README under **Screenshots** (replacing the steps above) so the previews appear inline — or keep this checklist-only README until the PNGs are ready.
...
| [`assets/screenshots/`](assets/screenshots/) | Redaction rules; **[`EMBED.md`](assets/screenshots/EMBED.md)** — paste-ready README image grid after PNGs exist |
